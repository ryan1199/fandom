<div wire:ignore x-cloak x-show="account_modal" x-transition.duration.500ms.scale.origin id="accountSettings" class="w-10/12 md:w-1/3 h-fit p-2 {{ 'bg-[' . $preferences['color_primary'] . ']/10' }} backdrop-blur-sm border-0 rounded-lg absolute z-10" :style="{ left: {{ $account_settings_modal_position['left'] }}+'px', right: {{ $account_settings_modal_position['right'] }}+'px',top: {{ $account_settings_modal_position['top'] }}+'px' ,bottom: {{ $account_settings_modal_position['bottom'] }}+'px' }">
    <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 {{ 'bg-[' . $preferences['color_secondary'] . ']' }} border-0 rounded-lg">
        ...
        {{-- rule --}}
        <div class="w-full h-full {{ 'text-[calc(' . $preferences['font_size'] . 'px-2px)]' }} {{ 'bg-[' . $preferences['color_primary'] . ']' }} rounded-lg select-none">
        ...
        <button type="submit" wire:click="deleteAccount" wire:confirm="Are you sure you want to delete your account?" class="w-full h-fit p-2 text-center {{ 'text-[' . $preferences['font_size'] . 'px]' }} text-black bg-red-500 border-0 rounded-lg cursor-pointer select-none transition-all duration-100 hover:opacity-50 active:duration-75 active:scale-[.95]">Delete Account</button>
    </div>
</div>
@script
    <script>
