<div wire:ignore x-cloak x-show="account_modal" x-transition.duration.500ms.scale.origin id="accountSettings"
    ...
    :style="{ left: {{ $account_settings_modal_position['left'] }}+'px', right: {{ $account_settings_modal_position['right'] }}+'px',top: {{ $account_settings_modal_position['top'] }}+'px' ,bottom: {{ $account_settings_modal_position['bottom'] }}+'px' }">
    <div
        ...
        <div
            ...
            <div class="p-2">
                <svg x-on:click="account_modal = false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                    fill="currentColor"
        ...
        <div
            ...
            <div style="background-image: url('{{ asset('account-settings-cover-black.svg') }}')"
                class="w-full h-20 bg-repeat bg-center rounded-lg"></div>
        </div>
        {{-- rule --}}
        <div
            class="w-full h-full text-[calc({{$preferences['font_size'] . 'px-2px'}})] bg-[{{ $preferences['color_primary'] }}] rounded-lg select-none">
            <div
                class="w-full h-full p-2 flex flex-col space-x-0 space-y-2 bg-[{{ $preferences['color_primary'] }}] border border-[{{ $preferences['color_secondary'] }}] rounded-lg">
                <p class="text-[{{ $preferences['color_text'] }}] font-medium">Rules</p>
                <ul
                    class="list-inside list-decimal flex flex-col space-x-0 space-y-2 text-[{{ $preferences['color_text'] }}]">
        ...
        @endif
        <form wire:submit="updateAccount" method="post"
                ...
                <label for="password"
                    ...
                    <div x-data="{ type: 'password', get isPassword() { return this.type === 'password' }, get isText() { return this.type === 'text' }}"
                        ...
                        <input wire:model="password" :type="type" id="password"
                            ...
                            required>
                        <svg x-show="isPassword" x-on:click="type = 'text'" xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24" fill="currentColor"
                            class="w-10 h-10 aspect-square p-2 text-[{{ $preferences['color_text'] }}] border border-[{{ $preferences['color_secondary'] }}] rounded-lg cursor-pointer transition-all duration-100 hover:opacity-50 active:duration-75 active:scale-[.95]">
                            <path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" />
                            <path fill-rule="evenodd"
                                d="M1.323 11.447C2.811 6.976 7.028 3.75 12.001 3.75c4.97 0 9.185 3.223 10.675 7.69.12.362.12.752 0 1.113-1.487 4.471-5.705 7.697-10.677 7.697-4.97 0-9.186-3.223-10.675-7.69a1.762 1.762 0 0 1 0-1.113ZM17.25 12a5.25 5.25 0 1 1-10.5 0 5.25 5.25 0 0 1 10.5 0Z"
                                clip-rule="evenodd" />
                        </svg>
                        <svg x-show="isText" x-on:click="type = 'password'" xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24" fill="currentColor"
                            class="w-10 h-10 aspect-square p-2 text-[{{ $preferences['color_text'] }}] border border-[{{ $preferences['color_secondary'] }}] rounded-lg cursor-pointer transition-all duration-100 hover:opacity-50 active:duration-75 active:scale-[.95]">
                            <path
        ...
        <button type="submit" wire:click="deleteAccount" wire:confirm="Are you sure you want to delete your account?"
            class="w-full h-fit p-2 text-center text-[{{ $preferences['font_size'] .'px' }}] text-black bg-red-500 border-0 border-transparent rounded-lg cursor-pointer select-none transition-all duration-100 hover:opacity-50 active:duration-75 active:scale-[.95]">Delete
            Account</button>
    </div>
</div>
@script
<script>
    ...
    accountSettingsHeader.addEventListener('mouseup', mouseUp, false);
