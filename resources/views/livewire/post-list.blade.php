<div class="w-full h-full flex flex-col space-x-0 space-y-2 {{ 'text-[' . $preferences['font_size'] . 'px]' }} {{ 'leading-[calc(' . $preferences['font_size'] . 'px*1.2)]' }} {{ 'font-[' . $preferences['selected_font_family'] . ']' }} text-zinc-500">
    @if ($from == 'post')
        @foreach ($posts as $post)
            <div wire:key="{{ 'post' . $post->id }}" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 border border-zinc-200 rounded-lg">
                <div class="flex flex-row justify-between items-center">
                    <div class="{{ 'hover:text-[' . $preferences['color_2'] . ']' }} animation-button">
                        <h1><a href="{{ route('post.show', $post) }}" class="font-semibold cursor-pointer" draggable="false">{{ $post->title }}</a></h1>
                    </div>
                    <div class="flex flex-row space-x-2 space-y-0">
                        <svg wire:click="editPost({{ $post->id }})" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6 {{ 'hover:text-[' . $preferences['color_2'] . ']' }} cursor-pointer animation-button">
                            <path d="M21.731 2.269a2.625 2.625 0 0 0-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 0 0 0-3.712ZM19.513 8.199l-3.712-3.712-12.15 12.15a5.25 5.25 0 0 0-1.32 2.214l-.8 2.685a.75.75 0 0 0 .933.933l2.685-.8a5.25 5.25 0 0 0 2.214-1.32L19.513 8.2Z" />
                        </svg>
                        <svg wire:click="deletePost({{ $post->id }})" wire:confirm="Are you sure you want to delete this post?" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6 {{ 'hover:text-[' . $preferences['color_2'] . ']' }} cursor-pointer animation-button">
                ...
                <div class="flex flex-col space-x-0 space-y-1 select-none">
                    @if ($post->publish_id == null)
                        <div x-data="{ {{ 'open_' . $post->id }}: false }" class="flex flex-col space-x-0 space-y-1">
                            <div x-on:click="{{ 'open_' . $post->id }} = ! {{ 'open_' . $post->id }}" class="w-fit h-fit mx-auto p-2 text-center font-semibold {{ 'hover:text-[' . $preferences['color_2'] . ']' }} cursor-pointer animation-button">Publish On</div>
                            <div x-cloak x-show="{{ 'open_' . $post->id }}" class="flex flex-col space-x-0 space-y-1 rounded-lg">
                                @foreach ($publish_on as $array)
                                    @if ($array['from'] == 'user')
                                        <div wire:key="{{ 'publishOnUser' . $array['data']->id }}" class="w-full h-fit p-2 bg-zinc-50 border border-zinc-200 rounded-lg">
                                            <div class="w-full h-fit flex flex-col space-x-0 space-y-2">
                                                ...
                                                <div class="w-full h-full flex flex-row space-x-2 space-y-0 text-left">
                                                    <div wire:click="publishPost({{ $post }}, '{{ $array['from'] }}', {{ Auth::id() }}, 'self')" class="w-fit h-fit p-1 bg-zinc-50 text-center border border-zinc-200 rounded-lg cursor-pointer {{ 'hover:border-[' . $preferences['color_2'] . ']' }} active:border-2 rounded-lg cursor-pointer animation-button">Self</div>
                                                    <div wire:click="publishPost({{ $post }}, '{{ $array['from'] }}', {{ Auth::id() }}, 'friend')" class="w-fit h-fit p-1 bg-zinc-50 text-center border border-zinc-200 rounded-lg cursor-pointer {{ 'hover:border-[' . $preferences['color_2'] . ']' }} active:border-2 rounded-lg cursor-pointer animation-button">Friend</div>
                                                    <div wire:click="publishPost({{ $post }}, '{{ $array['from'] }}', {{ Auth::id() }}, 'public')" class="w-fit h-fit p-1 bg-zinc-50 text-center border border-zinc-200 rounded-lg cursor-pointer {{ 'hover:border-[' . $preferences['color_2'] . ']' }} active:border-2 rounded-lg cursor-pointer animation-button">Public</div>
                                                </div>
                                            </div>
                                        </div>
                                    @else
                                        <div wire:key="{{ 'publishOnFandom' . $array['data']->id }}" class="w-full h-fit p-2 bg-zinc-50 border border-zinc-200 rounded-lg">
                                            <div class="w-full h-fit flex flex-col space-x-0 space-y-2">
                                                <div class="{{ 'text-[calc(theme(fontSize.lg)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)]' }} {{ 'leading-[calc(calc(theme(fontSize.lg)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)*1.2)]' }} font-extrabold">
                                                ...
                                                <div class="w-full h-full flex flex-row space-x-2 space-y-0 text-left">
                                                    <div wire:click="publishPost({{ $post }}, '{{ $array['from'] }}', {{ $array['data']->id }}, 'member')" class="w-fit h-fit p-1 bg-zinc-50 text-center border border-zinc-200 rounded-lg cursor-pointer {{ 'hover:border-[' . $preferences['color_2'] . ']' }} active:border-2 rounded-lg cursor-pointer animation-button">Member</div>
                                                    <div wire:click="publishPost({{ $post }}, '{{ $array['from'] }}', {{ $array['data']->id }}, 'public')" class="w-fit h-fit p-1 bg-zinc-50 text-center border border-zinc-200 rounded-lg cursor-pointer {{ 'hover:border-[' . $preferences['color_2'] . ']' }} active:border-2 rounded-lg cursor-pointer animation-button">Public</div>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                    ...
                    @else
                        <div wire:click="unpublishPost({{ $post->id }})" wire:confirm="Are you sure you want to unpublish this post?" class="w-fit h-fit mx-auto p-2 text-center font-semibold {{ 'hover:text-[' . $preferences['color_2'] . ']' }} cursor-pointer animation-button">Unpublished</div>
                    @endif
                </div>
            </div>
        @endforeach
