<div x-data="{ open_discuss: @entangle('openDiscuss').live }" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 {{ 'text-[' . $preferences['font_size'] . 'px]' }} {{ 'leading-[calc(' . $preferences['font_size'] . 'px*1.2)]' }} {{ 'font-[' . $preferences['selected_font_family'] . ']' }} {{ 'text-' . $preferences['color_2'] . '-900' }} {{ 'bg-' . $preferences['color_2'] . '-50/50' }} backdrop-blur-3xl shadow-sm {{ 'shadow-' . $preferences['color_2'] . '-900' }} rounded-lg">
    <div class="w-full h-fit flex flex-col space-x-0 space-y-2 select-none">
        ...
        <div x-on:click="open_discuss = ! open_discuss" class="w-full h-fit flex flex-row space-x-2 space-y-0 items-center">
        ...
        <div x-cloak x-show="!open_discuss" class="flex flex-col space-x-0 space-y-0">
    ...
    <div x-cloak x-show="open_discuss" class="w-full h-fit flex flex-col space-x-0 space-y-1 relative">
        @if ($messages->isNotEmpty())
            ...
            <div class="w-full h-fit max-h-[25vh] flex flex-col-reverse space-x-0 space-y-1 space-y-reverse overflow-x-clip overflow-y-auto">
                @foreach ($messages as $message)
                    @if ($message->user->id == Auth::id())
                        <div wire:key="{{ 'fandom-' . $fandom->id . '-discuss-' . $discuss->id . '-message-' . $message->id }}" class="w-full max-w-[80%] h-fit self-end flex flex-col space-x-0 space-y-1 border {{ 'border-' . $preferences['color_2'] . '-200' }} rounded-lg">
                    ...
                    @else
                        <div wire:key="{{ 'fandom-' . $fandom->id . '-discuss-' . $discuss->id . '-message-' . $message->id }}" class="w-full max-w-[80%] h-fit flex flex-col space-x-0 space-y-1 border {{ 'border-' . $preferences['color_2'] . '-200' }} rounded-lg">
                            <div class="w-full h-fit bg-gradient-to-tr {{ 'from-' . $preferences['color_1'] . '-900' }} {{ 'via-' . $preferences['color_2'] . '-900' }} {{ 'to-' . $preferences['color_3'] . '-900' }} {{ 'selection:bg-' . $preferences['color_2'] . '-50' }} {{ 'selection:text-' . $preferences['color_2'] . '-500' }} rounded-t-lg">
                                <div
                                    @if ($message->user->cover != null)
                                        style="background-image: url('{{ asset('storage/avatars/'.$message->user->avatar->image->url) }}')"
        ...
        @auth
            <div class="sticky bottom-0">
                @livewire(DiscussForm::class, ['discuss' => $discuss->id, 'preferences' => $preferences], key('discuss-form-for-discuss-' . $discuss->id . '-fandom-' . $fandom->id))
            </div>
        @endauth
    </div>
</div>