<div class="w-full h-screen max-h-[100vh] {{ 'text-[' . $preferences['font_size'] . 'px]' }} {{ 'leading-[calc(' . $preferences['font_size'] . 'px*1.2)]' }} {{ 'font-[' . $preferences['selected_font_family'] . ']' }} {{ 'text-' . $preferences['color_2'] . '-900' }} select-none overflow-clip">
    <div class="w-full h-fit max-h-[100vh] py-8 flex flex-col space-x-0 space-y-8 items-center overflow-y-auto">
        <div class="w-full sm:w-11/12 md:w-10/12 lg:w-8/12 h-fit self-center flex flex-col 2xl:flex-row space-x-0 space-y-8 2xl:space-x-8 2xl:space-y-0">
            <img src="{{ asset('storage/galleries/'.$gallery->image->url) }}" alt="{{ asset('storage/galleries/'.$gallery->image->url) }}" class="w-full max-w-3xl mx-auto 2xl:mx-0 p-2 object-cover shadow-lg {{ 'shadow-' . $preferences['color_2'] . '-900' }} rounded-3xl" draggable="false">
            <div class="w-full 2xl:w-fit h-fit p-2 flex flex-col space-x-0 space-y-2 tracking-widest shadow-lg {{ 'shadow-' . $preferences['color_2'] . '-900' }} rounded-3xl">
                    ...
                    <div class="flex flex-row space-x-2 space-y-0 items-stretch">
                        <div class="w-fit h-fit flex flex-row space-x-2 space-y-0 items-center justify-center {{ 'bg-' . $preferences['color_2'] . '-50' }} rounded-lg">
                            @if (in_array(Auth::id(), collect($gallery->rates)->where('like', true)->pluck('user_id')->toArray()))
                                ...
                                <div class="pr-2 font-semibold">{{ collect($gallery->rates)->where('like', true)->count() }}</div>
                            @else
                                ...
                                <div class="pr-2">{{ collect($gallery->rates)->where('like', true)->count() }}</div>
                            @endif
                        </div>
                        <div class="w-fit h-fit flex flex-row space-x-2 space-y-0 items-center justify-center {{ 'bg-' . $preferences['color_2'] . '-50' }} rounded-lg">
                            @if (in_array(Auth::id(), collect($gallery->rates)->where('dislike', true)->pluck('user_id')->toArray()))
                                ...
                                <div class="pr-2 font-semibold">{{ collect($gallery->rates)->where('dislike', true)->count() }}</div>
                            @else    
                                ...
                                <div class="pr-2">{{ collect($gallery->rates)->where('dislike', true)->count() }}</div>
                            @endif
                        </div>
                    </div>
                    <div class="w-fit h-fit flex flex-row space-x-2 space-y-0 items-center justify-center {{ 'bg-' . $preferences['color_2'] . '-50' }} rounded-lg">
                        ...
                        <div class="pr-2">{{ collect($gallery->comments)->count() }}</div>
                    </div>
                @endauth
            </div>
        </div>
        ...
        @if ($recommends['tags'] != null)
            <div class="w-full sm:w-11/12 md:w-10/12 lg:w-8/12 h-fit mx-auto p-2 flex flex-col space-x-0 space-y-2 tracking-widest shadow-lg {{ 'shadow-' . $preferences['color_2'] . '-900' }} rounded-3xl">
                <div class="w-fit h-fit p-2 {{ 'text-[calc(theme(fontSize.2xl)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)]' }} {{ 'leading-[calc(calc(theme(fontSize.2xl)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)*1.2)]' }} font-extrabold {{ 'bg-' . $preferences['color_2'] . '-50' }} rounded-3xl">
                    ...
                    @foreach ($recommends['tags'] as $gallery_1)
                        <a wire:navigate href="{{ route('gallery.show', $gallery_1) }}" draggable="false">
                            <img src="{{ asset('storage/galleries/'.$gallery_1->image->url) }}" alt="{{ asset('storage/galleries/'.$gallery_1->image->url) }}"
                                class="w-full h-40 object-cover rounded-3xl" draggable="false">
                        </a>
                    @endforeach
                </div>
            </div>
