    ...
    <div class="container h-fit max-h-[calc(100%-48px)] overflow-clip">
        <div
            ...
            <div wire:scroll
                    ...
                    <div class="w-full h-fit flex flex-col space-x-0 space-y-2">
                        ...
                        <p class="w-fit h-fit p-2 flex flex-row space-x-2 items-center rounded-lg">
                            ...
                            <span class="w-fit h-fit p-2 flex flex-row space-x-2 items-center border rounded-lg">
                                ...
                                <a href="{{ route('user', $gallery->publish->publishable) }}">
                                    <img src="{{ asset('storage/avatars/'.$gallery->publish->publishable->avatar->image->url) }}" alt="" class="w-10 h-auto aspect-square object-cover rounded-full">
                                </a>
                                @else    
                                <span>
                        ...
                        <p class="w-fit h-fit p-2 flex flex-row space-x-2 items-center rounded-lg">
                            ...
                            <span class="w-fit h-fit p-2 flex flex-row space-x-2 items-center border rounded-lg">
                                ...
                                <a href="{{ route('user', $gallery->user) }}">
                                    <img src="{{ asset('storage/avatars/'.$gallery->user->avatar->image->url) }}" alt="" class="w-10 h-auto aspect-square object-cover rounded-full">
                                </a>
                            </span>
                        </p>
                    </div>
                ...
                @if (class_basename($gallery->publish->publishable_type) === 'User')
                    <div class="w-full h-fit p-4 flex flex-col space-x-0 space-y-2 border border-[{{ $preferences['color_secondary'] }}] rounded-lg">
                        <div>Recommended pictures from <a href="{{ route('user', $gallery->publish->publishable) }}">{{ $gallery->publish->publishable->username }}</a></div>
                        <div class="grid grid-cols-5 gap-2">
                ...
                @else
                <div class="w-full h-fit p-4 flex flex-col space-x-0 space-y-2 border border-[{{ $preferences['color_secondary'] }}] rounded-lg">
                    <div>Recommended pictures from <a href="{{ route('fandom-details', $gallery->publish->publishable) }}">{{ $gallery->publish->publishable->name }}</a></div>
                    <div class="grid grid-cols-5 gap-2">
                        @foreach ($recommends['fandom'] as $gallery)
                            <a href="{{ route('gallery.show', $gallery) }}">
                                <img src="{{ asset('storage/galleries/'.$gallery->image->url) }}" alt="" class="w-full h-40 object-cover rounded-lg">
                ...
                <div class="w-full h-fit p-4 flex flex-col space-x-0 space-y-2 border border-[{{ $preferences['color_secondary'] }}] rounded-lg">
                    <div>Recommended pictures from <a href="{{ route('user', $gallery->user) }}">{{ $gallery->user->username }}</a></div>
                    <div class="grid grid-cols-5 gap-2">
