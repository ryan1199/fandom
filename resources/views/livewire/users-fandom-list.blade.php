<div class="w-full max-w-full h-fit flex flex-row space-x-2 space-y-0 overflow-clip overflow-x-auto">
    @foreach ($members as $member)
        <div wire:key="{{ 'user-' . $user->id . '-fandom-list-fandom-' . $member->fandom->id }}" class="shrink-0 w-full h-fit bg-gradient-to-tr {{ 'from-' . $preferences['color_1'] . '-900' }} {{ 'via-' . $preferences['color_2'] . '-900' }} {{ 'to-' . $preferences['color_3'] . '-900' }} {{ 'selection:bg-' . $preferences['color_2'] . '-50' }} {{ 'selection:text-' . $preferences['color_2'] . '-500' }} rounded-lg">
            <div
