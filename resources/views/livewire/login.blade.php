<div
    class="w-screen h-screen max-w-sm md:max-w-lg max-h-[100vh] mx-auto p-2 flex flex-col space-x-0 space-y-2 justify-center items-center relative z-0">
    <livewire:alert />
    <div class="w-full h-fit max-h-[calc(100%-64px-16px)] overflow-clip">
        <div class="w-full h-fit p-4 bg-white/10 backdrop-blur-sm border-0 border-transparent rounded-lg">
            <div wire:scroll
                class="w-full h-full max-h-[calc(100vh-64px-48px-16px)] pr-1 font-mono overflow-y-scroll overflow-x-clip">
                <div
                    ...
                    <div class="bg-gradient-to-tr from-orange-500 via-pink-500 to-indigo-500 rounded-lg select-none">
                        <div style="background-image: url('{{ asset('login-cover-black.svg') }}')"
                        ...
                        <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 justify-center">
                            <label for="username"
                                ...
                                <input wire:model="username" type="text" id="username"
                                    class="form-input accent-black caret-black basis-8/12 md:basis-9/12 border @error('username') invalid @else valid @enderror rounded-lg"
                                    required>
                            </label>
                            <label for="password"
                                class="flex flex-col md:flex-row space-x-0 space-y-2 md:space-x-2 space- md:space-y-0 justify-between items-stretch md:items-center">
                                ...
                                <div x-data="{ type: 'password', get isPassword() { return this.type === 'password' }, get isText() { return this.type === 'text' }}"
                                    ...
                                    <input wire:model="password" :type="type" id="password"
                                        class="w-full form-input accent-black caret-black border @error('password') invalid @else valid @enderror rounded-lg"
                                        required>
                            ...
                            <div
                                class="flex flex-col md:flex-row space-x-0 md:space-x-2 space-y-2 md:space-y-0 items-stretch md:items-center">
                                <label for="remember"
                                    class="w-fit h-fit p-2 shrink-0 flex flex-row space-x-2 space-y-0 justify-start items-center border @error('remember') invalid @else valid @enderror rounded-lg transition-all duration-100 hover:opacity-50 active:duration-75 active:scale-[.95]">
                                    <input wire:model="remember" type="checkbox" id="remember"
                                        class="p-2 form-checkbox accent-black border border-black rounded-sm cursor-pointer">
                                    <span
                                        class="text-black text-base font-medium text-left cursor-pointer select-none">Remember
                                        me</span>
                                </label>
                                <button type="submit"
                                    class="w-full h-fit self-center p-2 text-base text-black bg-white border border-black rounded-lg select-none transition-all duration-100 hover:opacity-50 active:duration-75 active:scale-[.95]">Login</button>
                            </div>
                        </div>
                    </form>
