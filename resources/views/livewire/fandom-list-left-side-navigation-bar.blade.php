<div x-data="{ {{ 'open_fandom_' . $fandom->id }}: false }" class="flex flex-col space-x-0 space-y-1">
    ...
    <div x-transition x-cloak x-show="{{ 'open_fandom_' . $fandom->id }}" class="pl-1 flex flex-col space-x-0 space-y-1">
        @if ($member->role->name == 'Manager')
            <div x-data="{ {{ 'open_setting_fandom_' . $fandom->id }}: false }" class="flex flex-col space-x-0 space-y-1">
                <div class="flex flex-row justify-between items-center">
                    <p>Setting</p>
                    <svg x-cloak x-show="!{{ 'open_setting_fandom_' . $fandom->id }}" x-on:click="{{ 'open_setting_fandom_' . $fandom->id }} = true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
                        <path fill-rule="evenodd" d="M12.53 16.28a.75.75 0 0 1-1.06 0l-7.5-7.5a.75.75 0 0 1 1.06-1.06L12 14.69l6.97-6.97a.75.75 0 1 1 1.06 1.06l-7.5 7.5Z" clip-rule="evenodd" />
                    </svg>
                    <svg x-cloak x-show="{{ 'open_setting_fandom_' . $fandom->id }}" x-on:click="{{ 'open_setting_fandom_' . $fandom->id }} = false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6 {{ 'text-[' . $preferences['color_2'] . ']' }}">
                        <path fill-rule="evenodd" d="M11.47 7.72a.75.75 0 0 1 1.06 0l7.5 7.5a.75.75 0 1 1-1.06 1.06L12 9.31l-6.97 6.97a.75.75 0 0 1-1.06-1.06l7.5-7.5Z" clip-rule="evenodd" />
                    </svg>
                </div>
                <div x-transition x-cloak x-show="{{ 'open_setting_fandom_' . $fandom->id }}" class="flex flex-col space-x-0 space-y-1">
        ...
        <div x-data="{ {{ 'open_discusses_fandom_' . $fandom->id }}: false }" class="flex flex-col space-x-0 space-y-1">
            ...
            <div x-transition x-cloak x-show="{{ 'open_discusses_fandom_' . $fandom->id }}" class="flex flex-col space-x-0 space-y-1">
