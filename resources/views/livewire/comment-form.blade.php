        ...
        @if ($comment != null)
            <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-4 {{ 'bg-' . $preferences['color_2'] . '-50' }} rounded-lg">
                <div class="w-full flex flex-row space-x-2 space-y-0 justify-between items-center">
                    <div class="flex flex-row space-x-2 space-y-0 items-center justify-between">
                        @if ($comment->user->avatar != null)
                            <img src="{{ asset('storage/avatars/'.$comment->user->avatar->image->url) }}" alt="{{ $comment->user->username }}" title="{{ $comment->user->username }}" class="aspect-square w-auto h-[7vh] rounded-full object-cover" draggable="false">
                        @else
                    ...
                    <svg wire:click="cancelReply" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-8 h-8 self-start {{ 'hover:text-' . $preferences['color_2'] . '-500' }} cursor-pointer animation-button">
