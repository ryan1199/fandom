<div x-data="{ open_component: @entangle('open_component').live }" x-cloak x-show="open_component" x-transition class="w-full max-w-md h-screen max-h-[100vh] {{ 'text-[' . $preferences['font_size'] . 'px]' }} {{ 'leading-[calc(' . $preferences['font_size'] . 'px*1.2)]' }} {{ 'font-[' . $preferences['selected_font_family'] . ']' }} {{ 'text-' . $preferences['color_2'] . '-900' }} {{ 'bg-' . $preferences['color_2'] . '-50/70' }} backdrop-blur-3xl border-l-2 {{ 'border-' . $preferences['color_2'] . '-900' }} border-dashed shadow-sm {{ 'shadow-' . $preferences['color_2'] . '-900' }} select-none overflow-y-auto absolute top-0 right-0 z-20">
