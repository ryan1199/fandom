<div class="flex flex-col space-x-0 space-y-1">
    @forelse ($followed_users as $followed_user)
        <div wire:key="{{ 'user-' . $user->id . '-follow-user-' . $followed_user->id }}" class="w-full h-fit p-2 flex flex-row space-x-2 space-y-0 justify-between items-start {{ 'bg-' . $preferences['color_2'] . '-100' }} rounded-lg select-none">
