<div class="{{ 'text-[' . $preferences['font_size'] . 'px]' }} {{ 'leading-[calc(' . $preferences['font_size'] . 'px*1.2)]' }} {{ 'font-[' . $preferences['selected_font_family'] . ']' }} {{ 'text-' . $preferences['color_2'] . '-900' }}">
    <form wire:submit="submitDiscuss">
        ...
        <div class="w-full h-fit flex flex-col space-x-0 space-y-2">
            ...
            <div class="w-full h-fit flex flex-row space-x-1 space-y-0 items-center">
                <textarea wire:model.live="content" placeholder="Your message" title="Your message" cols="30" rows="1" class="w-full h-full form-textarea {{ 'placeholder:text-' . $preferences['color_2'] . '-900' }} {{ 'bg-' . $preferences['color_2'] . '-50/10' }} border @error('content') {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @enderror {{ 'hover:border-' . $preferences['color_2'] . '-500' }} {{ 'accent-' . $preferences['color_2'] . '-500' }} {{ 'caret-' . $preferences['color_2'] . '-500' }} {{ 'focus:border-' . $preferences['color_2'] . '-500' }} rounded-lg animation"></textarea>
                <button type="submit" title="Send your message" class="w-fit h-fit p-2 {{ 'bg-' . $preferences['color_2'] . '-50/10' }} border {{ 'border-' . $preferences['color_2'] . '-200' }} rounded-lg group {{ 'hover:border-' . $preferences['color_2'] . '-500' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 {{ 'text-' . $preferences['color_2'] . '-900' }} {{ 'group-hover:text-' . $preferences['color_2'] . '-500' }} animation-button-group">
