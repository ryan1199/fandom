<div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 {{ 'text-[' . $preferences['font_size'] . 'px]' }} {{ 'leading-[calc(' . $preferences['font_size'] . 'px*1.2)]' }} {{ 'font-[' . $preferences['selected_font_family'] . ']' }} text-zinc-500 bg-zinc-50 border border-zinc-200 rounded-lg">
    <div class="bg-gradient-to-tr {{ 'from-[' . $preferences['color_1'] . ']' }} {{ 'via-[' . $preferences['color_2'] . ']' }} to-[{{ $preferences['color_3'] . ']' }} rounded-lg select-none">
        <div style="background-image: url('{{ asset('profile-settings-cover-white.svg') }}')" class="w-full h-20 bg-repeat bg-center border border-zinc-200 rounded-lg"></div>
    </div>
    @if ($errors->any())
    ...
    <form wire:submit>
        <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 bg-zinc-50 border border-zinc-200 rounded-lg justify-center">
            <label for="cover" class="flex flex-col space-x-0 space-y-2 justify-between items-stretch">
                ...
                <input wire:model.blur="cover" name="cover" type="file" id="cover" class="w-full form-input file:text-zinc-50 file:text-center file:align-middle file:p-2 file:bg-gradient-to-tr {{ 'file:from-[' . $preferences['color_1'] . ']' }} {{ 'file:via-[' . $preferences['color_2'] . ']' }} {{ 'file:to-[' . $preferences['color_3'] . ']' }} file:border file:border-zinc-200 file:rounded-lg border @error('cover') border-rose-500 @else border-zinc-200 @enderror {{ 'accent-[' . $preferences['color_2'] . ']' }} {{ 'caret-[' . $preferences['color_2'] . ']' }} {{ 'hover:border-[' . $preferences['color_2'] . ']' }} {{ 'focus:border-[' . $preferences['color_2'] . ']' }} rounded-lg cursor-pointer file:cursor-pointer animation animation-button-file">
            </label>
            <label for="avatar" class="flex flex-col space-x-0 space-y-2 justify-between items-stretch">
                ...
                <input wire:model.blur="avatar" name="avatar" type="file" id="avatar" class="w-full form-input file:text-zinc-50 file:text-center file:align-middle file:p-2 file:bg-gradient-to-tr {{ 'file:from-[' . $preferences['color_1'] . ']' }} {{ 'file:via-[' . $preferences['color_2'] . ']' }} {{ 'file:to-[' . $preferences['color_3'] . ']' }} file:border file:border-zinc-200 file:rounded-lg border @error('avatar') border-rose-500 @else border-zinc-200 @enderror {{ 'accent-[' . $preferences['color_2'] . ']' }} {{ 'caret-[' . $preferences['color_2'] . ']' }} {{ 'hover:border-[' . $preferences['color_2'] . ']' }} {{ 'focus:border-[' . $preferences['color_2'] . ']' }} rounded-lg cursor-pointer file:cursor-pointer animation animation-button-file">
            </label>
            <label for="status" class="flex flex-col space-x-0 space-y-2 justify-between items-stretch">
                <span class="font-medium text-left select-none">Status</span>
                <textarea wire:model.blur="status" id="status" placeholder="What are you feeling right now ?" class="w-full form-textarea border @error('status') border-rose-500 @else border-zinc-200 @enderror {{ 'accent-[' . $preferences['color_2'] . ']' }} {{ 'caret-[' . $preferences['color_2'] . ']' }} {{ 'hover:border-[' . $preferences['color_2'] . ']' }} {{ 'focus:border-[' . $preferences['color_2'] . ']' }} rounded-lg animation"></textarea>
            </label>
            <label for="description" class="flex flex-col space-x-0 space-y-2 justify-between items-stretch">
                ...
                <textarea wire:model.blur="description" id="description" placeholder="Describe your self" class="w-full form-textarea border @error('status') border-rose-500 @else border-zinc-200 @enderror {{ 'accent-[' . $preferences['color_2'] . ']' }} {{ 'caret-[' . $preferences['color_2'] . ']' }} {{ 'hover:border-[' . $preferences['color_2'] . ']' }} {{ 'focus:border-[' . $preferences['color_2'] . ']' }} rounded-lg animation"></textarea>
            </label>
        </div>
    </form>
</div>