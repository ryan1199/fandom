<div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 {{ 'text-[' . $preferences['font_size'] . 'px]' }} {{ 'leading-[calc(' . $preferences['font_size'] . 'px*1.2)]' }} {{ 'font-[' . $preferences['selected_font_family'] . ']' }} {{ 'text-' . $preferences['color_2'] . '-900' }} {{ 'bg-' . $preferences['color_2'] . '-100' }} rounded-lg">
    ...
    <div class="w-full h-full flex flex-col space-x-0 space-y-2 {{ 'text-[calc(theme(fontSize.sm)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)]' }} {{ 'leading-[calc(calc(theme(fontSize.sm)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)*1.2)]' }} select-none">
        ...
        <ol class="pl-8 flex flex-col space-x-0 space-y-2 text-pretty {{ 'marker:text-' . $preferences['color_2'] . '-500' }} list-decimal list-outside">
            ...
            <li>Status maximum length is 100 characters</li>
    ...
    <form wire:submit>
        <div class="w-full h-fit flex flex-col space-x-0 space-y-2 rounded-lg justify-center">
            ...
            <label for="avatar" class="flex flex-col space-x-0 space-y-2 justify-between items-stretch">
                ...
                <input wire:model.blur="avatar" name="avatar" type="file" id="avatar" class="w-full form-input {{ 'bg-' . $preferences['color_2'] . '-50' }} {{ 'file:text-' . $preferences['color_2'] . '-900' }} file:text-center file:align-middle file:p-2 file:bg-gradient-to-tr {{ 'file:from-' . $preferences['color_1'] . '-500' }} {{ 'file:via-' . $preferences['color_2'] . '-500' }} {{ 'file:to-' . $preferences['color_3'] . '-500' }} file:rounded-lg border @error('avatar') {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @enderror {{ 'accent-' . $preferences['color_2'] . '-500' }} {{ 'caret-' . $preferences['color_2'] . '-500' }} {{ 'hover:border-' . $preferences['color_2'] . '-500' }} {{ 'focus:border-' . $preferences['color_2'] . '-500' }} rounded-lg cursor-pointer file:cursor-pointer animation animation-button-file">
            </label>
            <label for="status" class="flex flex-col space-x-0 space-y-2 justify-between items-stretch">
                ...
                <textarea wire:model.blur="status" id="status" placeholder="What are you feeling right now ?" class="w-full form-textarea {{ 'bg-' . $preferences['color_2'] . '-50' }} border @error('status') {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @enderror {{ 'accent-' . $preferences['color_2'] . '-500' }} {{ 'caret-' . $preferences['color_2'] . '-500' }} {{ 'hover:border-' . $preferences['color_2'] . '-500' }} {{ 'focus:border-' . $preferences['color_2'] . '-500' }} rounded-lg animation"></textarea>
            </label>
            <label for="description" class="flex flex-col space-x-0 space-y-2 justify-between items-stretch">
                ...
                <textarea wire:model.blur="description" id="description" placeholder="Describe your self" class="w-full form-textarea {{ 'bg-' . $preferences['color_2'] . '-50' }} border @error('description') {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @enderror {{ 'accent-' . $preferences['color_2'] . '-500' }} {{ 'caret-' . $preferences['color_2'] . '-500' }} {{ 'hover:border-' . $preferences['color_2'] . '-500' }} {{ 'focus:border-' . $preferences['color_2'] . '-500' }} rounded-lg animation"></textarea>
            </label>
        </div>
    </form>
</div>