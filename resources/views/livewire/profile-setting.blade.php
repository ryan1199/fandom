<div wire:ignore x-cloak x-show="profile_modal" x-transition.duration.500ms.scale.origin id="profileSettings" class="w-10/12 md:w-1/3 h-fit p-2 {{ 'bg-[' . $preferences['color_primary'] . ']/10' }} backdrop-blur-sm border-0 border-transparent rounded-lg absolute z-10" :style="{ left: {{ $profile_settings_modal_position['left'] }}+'px', right: {{ $profile_settings_modal_position['right'] }}+'px',top: {{ $profile_settings_modal_position['top'] }}+'px' ,bottom: {{ $profile_settings_modal_position['bottom'] }}+'px' }">
        ...
        @endif
            <form wire:submit class="w-full h-fit p-0 {{ 'bg-[' . $preferences['color_primary'] . ']' }} border {{ 'border-[' . $preferences['color_secondary'] . ']' }} rounded-lg">
                <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 justify-center">
                    ...
                    <label for="status" class="flex flex-col sm:flex-row space-x-0 space-y-2 sm:space-x-2 sm:space-y-0 justify-between items-stretch sm:items-center {{ 'text-[' . $preferences['color_text'] . ']' }} {{ 'text-[' . $preferences['font_size'] . 'px]' }}">
