<div class="w-full h-fit flex flex-col space-x-0 space-y-4">
    <div class="w-full max-w-full h-fit grid grid-cols-3 gap-4">
        @forelse ($galleries as $gallery)
            @livewire(FandomsGalleryCard::class, ['gallery' => $gallery->id, 'fandom' => $gallery->publish->publishable->slug, 'preferences' => $preferences], key('user-fandoms-gallery-card-for-gallery-' . $gallery->id . '-' . rand()))
        @empty
