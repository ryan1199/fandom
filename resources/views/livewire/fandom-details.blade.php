<div class="w-full h-screen {{ 'text-[' . $preferences['font_size'] . 'px]' }} {{ 'leading-[calc(' . $preferences['font_size'] . 'px*1.2)]' }} {{ 'font-[' . $preferences['selected_font_family'] . ']' }} {{ 'text-' . $preferences['color_2'] . '-900' }}">
    <div class="w-full h-screen p-2 grid grid-cols-12 grid-flow-row-dense auto-rows-max auto-cols-max gap-2 {{ 'bg-' . $preferences['color_2'] . '-50' }}">
        {{-- fandom details --}}
        <div class="w-full h-full max-h-[calc(100vh-16px)] col-span-12 lg:col-span-6 p-2 {{ 'bg-' . $preferences['color_2'] . '-100' }} rounded-lg overflow-y-auto">
            <div x-data="{ tab: @entangle('tab').live }" class="w-full h-fit flex flex-col space-x-0 space-y-2 rounded-lg">
                ...
                <div class="w-full h-[30vh] relative rounded-lg">
                    <img src="{{ asset('storage/covers/' . $fandom->cover->image->url) }}" alt="Cover image {{ $fandom->name }}" title="Cover image {{ $fandom->name }}" class="w-full h-[30vh] object-cover block rounded-lg select-none" draggable="false">
                    <div class="absolute left-0 top-0 w-full h-[30vh]">
                        <div class="w-full h-[30vh] p-4 flex flex-col justify-center items-center space-x-0 space-y-4">
                            <img src="{{ asset('storage/avatars/' . $fandom->avatar->image->url) }}" alt="Avatar image {{ $fandom->name }}" title="Avatar image {{ $fandom->name }}" class="block w-auto h-[50%] aspect-square object-cover rounded-full select-none" draggable="false">
                            <div class="w-full h-fit max-h-[15vh] p-2 {{ 'bg-' . $preferences['color_2'] . '-50/70' }} backdrop-blur-sm rounded-lg overflow-clip">
                                <div x-data="{ open: false }" class="flex flex-row space-x-1 space-y-0 justify-center items-center">
                                    ...
                                    @auth
                                        <span class="relative">
                                            <svg x-on:click="open = ! open" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6 bg-inherit {{ 'text-' . $preferences['color_2'] . '-500' }} border {{ 'border-' . $preferences['color_2'] . '-500' }} rounded-full">
                                                <path fill-rule="evenodd" d="M10.5 6a1.5 1.5 0 1 1 3 0 1.5 1.5 0 0 1-3 0Zm0 6a1.5 1.5 0 1 1 3 0 1.5 1.5 0 0 1-3 0Zm0 6a1.5 1.5 0 1 1 3 0 1.5 1.5 0 0 1-3 0Z" clip-rule="evenodd" />
                                            </svg>
                                            @if (in_array(Auth::id(), $members['id']))
                                                <div wire:click="leave" wire:confirm="Are you sure you want to leave this fandom?" x-cloak x-show="open" class="absolute -top-1 -right-[72px] w-fit h-fit mx-auto px-2 py-1 bg-inherit border {{ 'border-' . $preferences['color_2'] . '-500' }} rounded-full select-none cursor-pointer">Leave</div>
                                            @else
                                                <div wire:click="join" wire:confirm="Are you sure you want to join this fandom?" x-cloak x-show="open" class="absolute -top-1 -right-[72px] w-fit h-fit mx-auto px-2 py-1 bg-inherit border {{ 'border-' . $preferences['color_2'] . '-500' }} rounded-full select-none cursor-pointer">Join</div>
                                            @endif
                                        </span>
                                    @endauth
                                </div>
                ...
                {{-- nav --}}
                <div class="w-full h-fit select-none">
                ...
                <div class="w-full h-fit">
                            ...
                            <div class="w-full h-fit flex flex-col space-x-0 space-y-2">
                                @if (in_array(Auth::id(), $members['id']))
                                    @forelse ($posts['member'] as $post)
                                        <a wire:key="{{ 'post-member-' . $loop->index }}" href="{{ route('post.show', $post) }}" class="w-full h-fit p-2 {{ 'bg-' . $preferences['color_2'] . '-50' }} border {{ 'border-' . $preferences['color_2'] . '-200' }} group {{ 'hover:border-' . $preferences['color_2'] . '-500' }} rounded-lg cursor-pointer animation" draggable="false">
                                ...
                                @else
                                    @forelse ($posts['public'] as $post)
                                        <a wire:key="{{ 'post-public-' . $loop->index }}" href="{{ route('post.show', $post) }}" class="w-full h-fit p-2 {{ 'bg-' . $preferences['color_2'] . '-50' }} border {{ 'border-' . $preferences['color_2'] . '-200' }} group {{ 'hover:border-' . $preferences['color_2'] . '-500' }} rounded-lg cursor-pointer animation" draggable="false">
                                            <h1 class="font-semibold {{ 'group-hover:text-' . $preferences['color_2'] . '-500' }} cursor-pointer animation">{{ $post->title }}</h1>
                                            <p class="text-right {{ 'group-hover:text-' . $preferences['color_2'] . '-500' }} animation">Published {{ $post->publish->created_at->diffForHumans(['options' => null]) }}</p>
                                        </a>
                                    @empty
                            ...
                            <div class="w-full h-fit grid gap-2 grid-cols-3">
                                @if (in_array(Auth::id(), $members['id']))
                                    @forelse ($galleries['member'] as $gallery)
                                        <div wire:key="{{ 'gallery-member-' . $gallery->id }}" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 justify-between {{ 'bg-' . $preferences['color_2'] . '-50' }} border {{ 'border-' . $preferences['color_2'] . '-200' }} group {{ 'hover:border-' . $preferences['color_2'] . '-500' }} rounded-lg animation">
                                            <div class="flex flex-col space-x-0 space-y-2">
                                ...
                                @else
                                    @forelse ($galleries['public'] as $gallery)
                                        <div wire:key="{{ 'gallery-public' . $gallery->id }}" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 justify-between {{ 'bg-' . $preferences['color_2'] . '-50' }} border {{ 'border-' . $preferences['color_2'] . '-200' }} group {{ 'hover:border-' . $preferences['color_2'] . '-500' }} rounded-lg animation">
                                            <div class="flex flex-col space-x-0 space-y-2">
                                                <a href="{{ route('gallery.show', $gallery) }}" draggable="false">
                                                    <img src="{{ asset('storage/galleries/'.$gallery->image->url) }}" alt="{{ asset('storage/galleries/'.$gallery->image->url) }}" class="w-full h-40 object-cover object-center rounded-lg" draggable="false">
                                                </a>
                    ...
                    <div x-transition x-cloak x-show="tab == 'post'" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2">
                    ...
                    <div x-transition x-cloak x-show="tab == 'gallery'" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2">
                        <div class="w-full h-fit flex flex-row justify-between items-center">
                            <div class="{{ 'text-[calc(theme(fontSize.4xl)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)]' }} {{ 'leading-[calc(calc(theme(fontSize.4xl)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)*1.2)]' }} font-extrabold">
                                <span class="bg-clip-text text-transparent bg-gradient-to-tr {{ 'from-' . $preferences['color_1'] . '-900' }} {{ 'via-' . $preferences['color_2'] . '-900' }} {{ 'to-' . $preferences['color_3'] . '-900' }}">
                                  Gallery
                    ...
                    <div x-transition x-cloak x-show="tab == 'user'" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2">
                        <div class="{{ 'text-[calc(theme(fontSize.4xl)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)]' }} {{ 'leading-[calc(calc(theme(fontSize.4xl)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)*1.2)]' }} font-extrabold">
                            <span class="bg-clip-text text-transparent bg-gradient-to-tr {{ 'from-' . $preferences['color_1'] . '-900' }} {{ 'via-' . $preferences['color_2'] . '-900' }} {{ 'to-' . $preferences['color_3'] . '-900' }}">
                              User
                            </span>
                        ...
                        {{-- manager --}}
                        <div class="w-full h-fit p-1 flex flex-col space-x-0 space-y-1 {{ 'bg-' . $preferences['color_2'] . '-50' }} border {{ 'border-' . $preferences['color_2'] . '-200' }} rounded-lg">
                            ...
                            <div class="flex flex-row flex-wrap">
                                @foreach ($members['manager']['list'] as $manager)
                                    <div wire:key="{{ 'user-manager-' . $loop->index }}" class="w-fit m-1">
                                        <a href="{{ route('user', $manager->user) }}" class="p-2 flex flex-row space-x-2 space-y-0 justify-start items-center {{ 'bg-' . $preferences['color_2'] . '-100' }} border {{ 'border-' . $preferences['color_2'] . '-200' }} group {{ 'hover:border-' . $preferences['color_2'] . '-500' }} rounded-lg animation" title="{{ $manager->user->username }}" draggable="false">
                        ...
                        {{-- member --}}
                        <div class="w-full h-fit p-1 flex flex-col space-x-0 space-y-1 {{ 'bg-' . $preferences['color_2'] . '-50' }} {{ 'border-' . $preferences['color_2'] . '-200' }} border {{ 'border-' . $preferences['color_2'] . '-200' }} rounded-lg">
                            <h3 class="text-center">Members</h3>
                            <hr class="{{ 'border-' . $preferences['color_2'] . '-200' }}">
                            <div class="flex flex-row flex-wrap">
                                @foreach ($members['member']['list'] as $member)
                                    <div wire:key="{{ 'user-member-' . $loop->index }}" class="w-fit m-1">
                                        <a href="{{ route('user', $member->user) }}" class="p-2 flex flex-row space-x-2 space-y-0 justify-start items-center {{ 'bg-' . $preferences['color_2'] . '-100' }} border {{ 'border-' . $preferences['color_2'] . '-200' }} group {{ 'hover:border-' . $preferences['color_2'] . '-500' }} rounded-lg animation" title="{{ $member->user->username }}" draggable="false">
                    ...
                    @if (in_array(Auth::id(), $members['manager']['id']))
                        <div x-transition x-cloak x-show="tab == 'setting'" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2">
                            ...
                            <div class="w-full h-fit p-2 {{ 'bg-' . $preferences['color_2'] . '-50' }} rounded-lg">
        ...
        {{-- discusses --}}
        <div @if (in_array(Auth::id(), $members['manager']['id'])) x-data="{ openDiscussCreateComponent: false }" @endif class="w-full h-fit max-h-[calc(100vh-16px)] p-2 col-span-12 lg:col-span-6 flex flex-col space-x-0 space-y-2 {{ 'bg-' . $preferences['color_2'] . '-100' }} rounded-lg overflow-y-auto">
            ...
            @if (in_array(Auth::id(), $members['manager']['id']))
                <div x-cloak x-show="openDiscussCreateComponent" class="w-full h-fit p-2 {{ 'bg-' . $preferences['color_2'] . '-50' }} rounded-lg">
