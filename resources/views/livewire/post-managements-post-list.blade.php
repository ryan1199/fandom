    ...
    @forelse ($posts as $post)
        @if ($post->publish != null)
            @if (class_basename($post->publish->publishable_type) === 'User')
                <div wire:key="{{ 'post-management-for-post-' . $post->id . '-' . rand() }}" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 {{ 'bg-' . $preferences['color_2'] . '-950' }} backdrop-blur-xl {{ 'text-' . $preferences['color_2'] . '-100' }} rounded-lg">
                    @livewire(UsersPostCard::class, ['post' => $post->id, 'user' => $post->publish->publishable->username, 'preferences' => $preferences], key('post-card-for-post-' . $post->id . '-' . rand()))
            ...
            @else
                <div wire:key="{{ 'post-management-for-post-' . $post->id . '-' . rand() }}" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 {{ 'bg-' . $preferences['color_2'] . '-950' }} backdrop-blur-xl {{ 'text-' . $preferences['color_2'] . '-100' }} rounded-lg">
                    @livewire(FandomsPostCard::class, ['post' => $post->id, 'fandom' => $post->publish->publishable->slug, 'preferences' => $preferences], key('post-card-for-post-' . $post->id . '-' . rand()))
                    <div class="flex flex-row justify-between items-start">
                        <div class="flex flex-col space-x-0 space-y-2">
                            <p class="text-left"><span class="font-semibold">Created</span> by {{ $post->user->username }} {{ $post->created_at->diffForHumans(['options' => null]) }}</p>
                            <p class="text-left">
        ...
        @else
            <div wire:key="{{ 'post-management-for-post-' . $post->id . '-' . rand() }}" class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 {{ 'bg-' . $preferences['color_2'] . '-950' }} backdrop-blur-xl {{ 'text-' . $preferences['color_2'] . '-100' }} rounded-lg">
                @livewire(PostCard::class, ['post' => $post->id, 'preferences' => $preferences], key('post-card-for-post-' . $post->id . '-' . rand()))
                <div class="flex flex-row justify-between items-start">
                    <div class="flex flex-col space-x-0 space-y-2">
                        <p class="text-left"><span class="font-semibold">Created</span> by {{ $post->user->username }} {{ $post->created_at->diffForHumans(['options' => null]) }}</p>
                        <p class="text-left">
                    ...
                    <div x-data="{ {{ 'open_publish_on_for_post_' . $post->id }}: false }" class="flex flex-col space-x-0 space-y-1">
                        ...
                        <div x-cloak x-show="{{ 'open_publish_on_for_post_' . $post->id }}" class="flex flex-col space-x-0 space-y-2 rounded-lg">
                            @foreach ($publish_on as $array)
                                @if ($array['from'] == 'user')
                                    <div wire:key="{{ 'post-list-from-post-management-publish-on-user-' . $array['data']->id }}" class="w-full h-fit p-2 {{ 'bg-' . $preferences['color_2'] . '-50' }} backdrop-blur-xl {{ 'text-' . $preferences['color_2'] . '-900' }} rounded-lg">
                                ...
                                @else
                                    <div wire:key="{{ 'post-list-from-post-management-publish-on-fandom-' . $array['data']->id }}" class="w-full h-fit p-2 {{ 'bg-' . $preferences['color_2'] . '-50' }} backdrop-blur-xl {{ 'text-' . $preferences['color_2'] . '-900' }} rounded-lg">
                                        <div class="w-full h-fit flex flex-col space-x-0 space-y-2">
                                            <div class="{{ 'text-[calc(theme(fontSize.lg)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)]' }} {{ 'leading-[calc(calc(theme(fontSize.lg)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)*1.2)]' }} font-extrabold">
