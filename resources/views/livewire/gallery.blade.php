        ...
        <div class="w-full lg:max-w-screen-sm h-fit max-h-[25vh] lg:max-h-[100vh] p-2 pb-0 lg:pb-2 pr-2 lg:pr-1 overflow-y-auto">
            <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 text-left bg-zinc-50 border border-zinc-200 rounded-lg">
                <div class="w-full h-fit flex flex-row justify-between items-center">
                    ...
                    <svg wire:click="createGallery" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-8 h-8 {{ 'hover:text-[' . $preferences['color_2'] . ']' }} cursor-pointer animation-button">
                        <path fill-rule="evenodd" d="M12 3.75a.75.75 0 0 1 .75.75v6.75h6.75a.75.75 0 0 1 0 1.5h-6.75v6.75a.75.75 0 0 1-1.5 0v-6.75H4.5a.75.75 0 0 1 0-1.5h6.75V4.5a.75.75 0 0 1 .75-.75Z" clip-rule="evenodd" />
                    </svg>
                </div>
                @livewire(GallerySearch::class, ['preferences' => $preferences, 'from' => 'user'])
                @livewire(GalleryList::class, ['preferences' => $preferences, 'from' => 'gallery'])
            </div>
        </div>
        @livewire(GalleryCreateEdit::class, ['preferences' => $preferences])
    </div>
</div>
