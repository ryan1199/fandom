            ...
            <div wire:scroll class="w-full h-full max-h-[calc(100vh-64px-48px-16px)] pr-1 font-mono overflow-y-auto overflow-x-clip">
                <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 bg-black border-0 border-transparent rounded-lg">
                    <div class="bg-gradient-to-tr from-orange-500 via-pink-500 to-indigo-500 rounded-lg select-none">
                        <div style="background-image: url('{{ asset('verification-cover-black.svg') }}')" class="w-full h-20 bg-repeat bg-center rounded-lg"></div>                    </div>
                    @if ($errors->any())
                    ...
                    <form wire:submit="sendEmailVerification" method="post" class="w-full h-fit p-0 bg-white border border-black rounded-lg">
                        ...
                        <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 justify-center">
                            ...
                            <button type="submit" class="w-full h-fit self-center p-2 text-base text-black bg-white border border-black rounded-lg select-none transition-all duration-100 hover:opacity-50 active:duration-75 active:scale-[.95]">Verification</button>
                        </div>
                    </form>
                </div>
            </div>
