                ...
                <div class="w-full h-full">
                    <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 {{ 'bg-[' . $preferences['color_secondary'] . ']' }} border-0 rounded-lg">
                        <div class="bg-gradient-to-tr {{ 'from-[' . $preferences['color_1'] . ']' }} {{ 'via-[' . $preferences['color_2'] . ']' }} {{ 'to-[' . $preferences['color_3'] . ']' }} rounded-lg select-none">
                            <div style="background-image: url('{{ asset('register-cover-black.svg') }}')" class="w-full h-20 bg-repeat bg-center rounded-lg"></div>
                        </div>
                        {{-- rule --}}
                        <div class="w-full h-full p-2 flex flex-col space-x-0 space-y-2 {{ 'text-[calc(' . $preferences['font_size'] . 'px-2px)]' }} {{ 'bg-[' . $preferences['color_primary'] . ']' }} border {{ 'border-[' . $preferences['color_secondary'] . ']' }} rounded-lg select-none">
                            ...
                            <div class="w-full h-full p-2 flex flex-col space-x-0 space-y-2 {{ 'bg-[' . $preferences['color_primary'] . ']' }} border {{ 'border-[' . $preferences['color_secondary'] . ']' }} rounded-lg">
                                <p class="{{ 'text-[' . $preferences['color_text'] . ']' }} font-medium">Important note</p>
                                <p class="{{ 'text-[' . $preferences['color_text'] . ']' }} font-medium">Please verify your email address after registration otherwise you can not login</p>
                            </div>
                        </div>
                        @if ($errors->any())
                        ...
                        @endif
                        <form wire:submit="register" method="post" class="w-full h-fit {{ 'bg-[' . $preferences['color_primary'] . ']' }} border {{ 'border-[' . $preferences['color_secondary'] . ']' }} rounded-lg">
                            @csrf
                            <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 justify-center">
                                <label for="username" class="flex flex-col md:flex-row space-x-0 space-y-2 md:space-x-2 space- md:space-y-0 justify-between items-stretch md:items-center">
                                    ...
                                    <input wire:model="username" type="text" id="username" class="form-input basis-8/12 md:basis-9/12 border @error('username') border-red-500 @else {{ 'border-[' . $preferences['color_secondary'] . ']' }} @enderror rounded-lg" required>
                                </label>
                                <label for="email" class="flex flex-col md:flex-row space-x-0 space-y-2 md:space-x-2 space- md:space-y-0 justify-between items-stretch md:items-center">
                                ...
                                <label for="password" class="flex flex-col md:flex-row space-x-0 space-y-2 md:space-x-2 space- md:space-y-0 justify-between items-stretch md:items-center">
                                    ...
                                    <div x-data="{ type: 'password', get isPassword() { return this.type === 'password' }, get isText() { return this.type === 'text' }}" class="basis-8/12 md:basis-9/12 flex flex-row space-x-2 space-y-0 items-center">
                                        <input wire:model="password" :type="type" id="password" class="w-full form-input border @error('password') border-red-500 @else {{ 'border-[' . $preferences['color_secondary'] . ']' }} @enderror rounded-lg" required>
                                ...
                                <label for="password_confirmation" class="flex flex-col md:flex-row space-x-0 space-y-2 md:space-x-2 space- md:space-y-0 justify-between items-stretch md:items-center">
                                    <span class="basis-2/12 {{ 'text-[' . $preferences['color_text'] . ']' }} font-medium text-left select-none">Confirm Password</span>
                                    <div x-data="{ type: 'password', get isPassword() { return this.type === 'password' }, get isText() { return this.type === 'text' }}" class="basis-8/12 md:basis-9/12 flex flex-row space-x-2 space-y-0 items-center">
                                        <input wire:model="password_confirmation" :type="type" id="password_confirmation" class="w-full form-input border @error('password_confirmation') border-red-500 @else {{ 'border-[' . $preferences['color_secondary'] . ']' }} @enderror rounded-lg" required>
                                        <svg x-show="isPassword" x-on:click="type = 'text'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-10 h-10 aspect-square p-2 border {{ 'border-[' . $preferences['color_secondary'] . ']' }} rounded-lg cursor-pointer transition-all duration-100 hover:opacity-50 active:duration-75 active:scale-[.95]">
                                            <path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" />
                                            <path fill-rule="evenodd" d="M1.323 11.447C2.811 6.976 7.028 3.75 12.001 3.75c4.97 0 9.185 3.223 10.675 7.69.12.362.12.752 0 1.113-1.487 4.471-5.705 7.697-10.677 7.697-4.97 0-9.186-3.223-10.675-7.69a1.762 1.762 0 0 1 0-1.113ZM17.25 12a5.25 5.25 0 1 1-10.5 0 5.25 5.25 0 0 1 10.5 0Z" clip-rule="evenodd" />
                                        </svg>
                                ...
                                <button type="submit" class="w-full h-fit self-center p-2 {{ 'text-[' . $preferences['color_text'] . ']' }} {{ 'bg-[' . $preferences['color_primary'] . ']' }} border {{ 'border-[' . $preferences['color_secondary'] . ']' }} rounded-lg select-none transition-all duration-100 hover:opacity-50 active:duration-75 active:scale-[.95]">Register</button>
                            </div>
                        </form>
