<div class="w-full h-fit {{ 'text-[' . $preferences['font_size'] . 'px]' }} {{ 'leading-[calc(' . $preferences['font_size'] . 'px*1.2)]' }} {{ 'font-[' . $preferences['selected_font_family'] . ']' }} {{ 'text-' . $preferences['color_2'] . '-900' }}">
    <div class="w-full h-fit flex flex-col space-x-0 space-y-2">
        ...
        <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 {{ 'bg-' . $preferences['color_2'] . '-50/50' }} backdrop-blur-3xl shadow-sm {{ 'shadow-' . $preferences['color_2'] . '-900' }} rounded-lg">
            ...
            @enderror
            <textarea wire:model="tags" class="w-full form-textarea {{ 'bg-' . $preferences['color_2'] . '-50/10' }} {{ 'placeholder:text-' . $preferences['color_2'] . '-900' }} {{ 'selection:bg-' . $preferences['color_2'] . '-500' }} {{ 'selection:text-' . $preferences['color_2'] . '-50' }} border @error('tags') {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @enderror {{ 'hover:border-' . $preferences['color_2'] . '-500' }} {{ 'accent-' . $preferences['color_2'] . '-500' }} {{ 'caret-' . $preferences['color_2'] . '-500' }} {{ 'focus:border-' . $preferences['color_2'] . '-500' }} rounded-lg animation" placeholder="Post Tags"></textarea>
        </div>
        <div class="w-full h-fit p-2 flex flex-col space-x-0 space-y-2 {{ 'bg-' . $preferences['color_2'] . '-50/50' }} backdrop-blur-3xl shadow-sm {{ 'shadow-' . $preferences['color_2'] . '-900' }} rounded-lg">
            ...
            <div class="w-full grid gap-2 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2">
                @foreach ($available_publish_on as $array)
                    @if ($array['from'] == 'user')
                        <div wire:key="{{ 'gallery-create-edit-publish-on-user-' . $array['data']->id }}" class="w-full h-fit p-2 border @if($publish_on['from'] == $array['from'] && $publish_on['name'] == Auth::user()->username) {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @endif rounded-lg">
                            <div class="w-full h-fit flex flex-col space-x-0 space-y-2">
                                ...
                                <div class="w-full h-full flex flex-row space-x-2 space-y-0 text-left">
                                    <div wire:click="setUploadLocation('{{ $array['from'] }}',{{ Auth::id() }},'{{ Auth::user()->username }}','self')" class="p-1 text-center border @if($visible == 'self' && $publish_on['from'] == $array['from'] && $publish_on['name'] == Auth::user()->username) {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @endif {{ 'hover:border-' . $preferences['color_2'] . '-500' }} active:border-2 rounded-lg cursor-pointer animation-button">self</div>
                                    <div wire:click="setUploadLocation('{{ $array['from'] }}',{{ Auth::id() }},'{{ Auth::user()->username }}','friend')" class="p-1 text-center border @if($visible == 'friend' && $publish_on['from'] == $array['from'] && $publish_on['name'] == Auth::user()->username) {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @endif {{ 'hover:border-' . $preferences['color_2'] . '-500' }} active:border-2 rounded-lg cursor-pointer animation-button">friend</div>
                                    <div wire:click="setUploadLocation('{{ $array['from'] }}',{{ Auth::id() }},'{{ Auth::user()->username }}','public')" class="p-1 text-center border @if($visible == 'public' && $publish_on['from'] == $array['from'] && $publish_on['name'] == Auth::user()->username) {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @endif {{ 'hover:border-' . $preferences['color_2'] . '-500' }} active:border-2 rounded-lg cursor-pointer animation-button">public</div>
                                </div>
                            </div>
                        </div>
                    @else
                        <div wire:key="{{ 'gallery-create-edit-publish-on-fandom-' . $array['data']->id }}" class="w-full h-fit p-2 border @if($publish_on['from'] == $array['from'] && $publish_on['name'] == $array['data']->name) {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @endif rounded-lg">
                            <div class="w-full h-fit flex flex-col space-x-0 space-y-2">
                                <div class="{{ 'text-[calc(theme(fontSize.lg)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)]' }} {{ 'leading-[calc(calc(theme(fontSize.lg)-theme(fontSize.base)+' . $preferences['font_size'] . 'px)*1.2)]' }} font-extrabold">
                                ...
                                <div class="w-full h-full flex flex-row space-x-2 space-y-0 text-left">
                                    <div wire:click="setUploadLocation('{{ $array['from'] }}',{{ $array['data']->id }},'{{ $array['data']->name }}','member')" class="p-1 text-center border @if($visible == 'member' && $publish_on['from'] == $array['from'] && $publish_on['name'] == $array['data']->name) {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @endif {{ 'hover:border-' . $preferences['color_2'] . '-500' }} active:border-2 rounded-lg cursor-pointer animation-button">member</div>
                                    <div wire:click="setUploadLocation('{{ $array['from'] }}',{{ $array['data']->id }},'{{ $array['data']->name }}','public')" class="p-1 text-center border @if($visible == 'public' && $publish_on['from'] == $array['from'] && $publish_on['name'] == $array['data']->name) {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @endif {{ 'hover:border-' . $preferences['color_2'] . '-500' }} active:border-2 rounded-lg cursor-pointer animation-button">public</div>
                                </div>
                            </div>
                        </div>
                    @endif
