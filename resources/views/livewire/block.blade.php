<div class="flex flex-col space-x-0 space-y-1">
    @forelse ($blocked_users as $blocked_user)
        <div wire:key="{{ 'user-' . $user->id . '-block-user-' . $blocked_user->id }}" class="w-full h-fit p-2 flex flex-row space-x-2 space-y-0 items-start {{ 'bg-' . $preferences['color_2'] . '-100' }} rounded-lg select-none">
