    ...
    <form wire:submit="createDiscussion">
        ...
        <div class="w-full h-fit flex flex-col space-x-0 space-y-2">
            <label for="name" class="flex flex-col space-x-0 space-y-2 justify-between items-stretch">
                ...
                <input wire:model="name" type="text" placeholder="Discussion Name" id="name" class="w-full h-fit form-input border {{ 'border-' . $preferences['color_2'] . '-200' }} {{ 'hover:border-' . $preferences['color_2'] . '-500' }} {{ 'accent-' . $preferences['color_2'] . '-500' }} {{ 'caret-' . $preferences['color_2'] . '-500' }} {{ 'focus:border-' . $preferences['color_2'] . '-500' }} rounded-lg animation">
            </label>
            <label for="visible" class="flex flex-col space-x-0 space-y-2 justify-between items-stretch">
                ...
                <select wire:model.live="visible" id="visible" class="w-full h-fit form-select border {{ 'border-' . $preferences['color_2'] . '-200' }} {{ 'hover:border-' . $preferences['color_2'] . '-500' }} {{ 'accent-' . $preferences['color_2'] . '-500' }} {{ 'caret-' . $preferences['color_2'] . '-500' }} {{ 'focus:border-' . $preferences['color_2'] . '-500' }} rounded-lg animation">
            ...
            <button type="submit" class="w-fit h-fit p-2 self-end text-nowrap font-semibold {{ 'hover:text-' . $preferences['color_2'] . '-500' }} {{ 'bg-' . $preferences['color_2'] . '-50' }} rounded-lg cursor-pointer select-none animation-button">Create</button>
        </div>
    </form>
</div>
