            ...
            <form wire:submit="sendEmailForgotPassword" method="post">
                ...
                <div class="w-full h-fit flex flex-col space-x-0 space-y-2 justify-center">
                    <label for="email" class="flex flex-col space-x-0 space-y-2">
                        ...
                        <input wire:model="email" type="email" id="email" class="w-full h-fit form-input border @error('email') {{ 'border-' . $preferences['color_2'] . '-500' }} @else {{ 'border-' . $preferences['color_2'] . '-200' }} @enderror {{ 'accent-' . $preferences['color_2'] . '-500' }} {{ 'caret-' . $preferences['color_2'] . '-500' }} {{ 'hover:border-' . $preferences['color_2'] . '-500' }} {{ 'focus:border-' . $preferences['color_2'] . '-500' }} animation rounded-lg" required>
                    </label>
                    <button type="submit" class="w-fit h-fit p-2 self-end font-semibold {{ 'hover:text-' . $preferences['color_2'] . '-500' }} {{ 'bg-' . $preferences['color_2'] . '-50' }} rounded-lg select-none animation-button hover:-translate-x-2">Reset Password</button>
                </div>
            </form>
        </div>
    </div>
